...
rfs=dlmread('SiC_specR.dat');%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%read data from tranmission spectrum saves as ASCII .dat file
d=size(rfs,1);
l0=rfs(1,1)-1;
...
figure; yyaxis left; plot(l,thetafin,'b');ylabel('Phase (radian)');yyaxis right;plot(l,R,'r'); ylabel('Specular Reflectance'); axis square; grid on; xlabel('wavelength');legend('Moissanite (SiC)');
figure; plot(l,n,'b');axis square; legend('DSKK estimate for Moissanite (SiC)');grid on; xlabel('wavelength'); ylabel('Refractive Index');
...
navg=sum(nfin)/300;%Mean value of DSKK refractive index over visible range 400 nm to 700 nm
