...
rfs=dlmread('QTZ_specR.dat');%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%read data from tranmission spectrum saves as ASCII .dat file
d=size(rfs,1);
l0=rfs(1,1)-1;
...
n2=1.5377;%known refractive index at wavelength of 832.50 nm 
n1=1.6509;%known refractive index at wavelength of 198.00 nm 


wl2=832.50;%known pivot point wavelength
wl1=198.00;%known pivot point wavelength


for i=1:d
kl(i,1)= (v(i,1)*l(i,1)*(1e-9))/(4*pi*2*7*1e-3);%Evaluating the extinction coefficient for a 1 mm thick slide
end
...
figure; yyaxis left; plot(l,thetafin,'b');ylabel('Phase (radian)');yyaxis right;plot(l,R,'r'); ylabel('Specular Reflectance'); axis square; grid on; xlabel('wavelength');legend('Quartz (SiO2)');
figure; plot(l,n,'b');axis square; legend('DSKK estimate for Quartz (SiO2)');grid on; xlabel('wavelength'); ylabel('Refractive Index');
